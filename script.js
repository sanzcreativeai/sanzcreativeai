...
// ---- HARD SAFETY NET ----
// Guarantees hero/card content is never permanently invisible, no matter what
// goes wrong elsewhere (Motion failing, a thrown error, a slow CDN, anything).
// This runs on a timer completely independent of the rest of the script.
setTimeout(() => {
...
// ---------- 3D tilt effect (cards lean toward cursor) ----------
// Excludes .work-box and .tool-badge — those already have continuous CSS
// float animations using transform, so adding JS tilt there would conflict.
if (matchMedia('(hover: hover)').matches && !prefersReducedMotion) {
  const tiltSelector = '.card, .price-card, .achieve, .browser-frame, .scene-frame';
...
if (!hasMotion) {
  ...
  if (logoRingCircle) {
    animate(
      ...
      { duration: 1.3, ease: EASE, delay: 0.15 }
    );
  }
  ...
  if (heroUnderlinePath) {
    ...
    animate(
      ...
      { duration: 0.9, ease: EASE, delay: 0.9 }
    );
  }
  ...
  if (heroEls.length) {
    animate(
      heroEls,
      { opacity: [0, 1], y: [26, 0] },
      { duration: 0.7, delay: stagger(0.12, { startDelay: 0.1 }), ease: EASE }
    );
  }
    ...
    inView(
      ...
      () => {
        animate(
          ...
          { duration: 0.55, delay: stagger(0.08), ease: EASE }
        );
      },
  ...
  const springSelector = '.price-cta, .site-link, .fab, .work-icon';
